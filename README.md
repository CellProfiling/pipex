...
---------
  ...
  - Quite fast: a full CODEX run of 20 antibodies with 6k resolution images takes 0min to fully process by PIPEX in 16Gb laptop (4min segmentation | 2min analysis | 1min QuPath conversion | 1min mask post-filtering).
...
- `analysis.py` : performs PIPEX broad scope analysis. MUST be run AFTER a segmentation. Uses the following parameters:
  ...
  - `-log_norm=<optional, yes or no to apply log n + 1 normalization>` : example -> -log_norm=yes. **OBS**: this will apply an additional log1p normalization to the markers intensities
