...
- `segmentation.py` : performs PIPEX segmentation. Uses the following parameters:
  ...
  - `-membrane_min_growth=<optional, minimum fractional area growth over the bare nucleus>` : example -> -membrane_min_growth=0.05. **OBS**: a membrane watershed cell is only trusted to reshape a nucleus if it grew more than this fraction beyond its seed nucleus. In zones where the membrane marker is not (or barely) expressed the watershed cannot flood outward, so the cell stays close to its nucleus size; those cells are ignored and the plain nuclei + expansion is kept instead. Where the membrane IS expressed, it carves the expansion inward while the expansion caps the growth outward. Default is 0.05 (5%). Lower values trust weaker membrane growth; higher values require the membrane to shape the cell more clearly.
